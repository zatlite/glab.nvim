...
M.browse_mr = function()
	local json = {
		query = 'query { project(fullPath: "'
			.. get_project_path()
			.. '") { mergeRequests(state: opened) { nodes { reference title sourceBranch webUrl }}}}',
	}
	local post_res = curl.post(state.GITLAB_URL .. "/api/graphql", {
		accept = "application/json",
		body = vim.fn.json_encode(json),
	...
	for i, node in ipairs(nodes) do
		table.insert(
			list,
			i,
			{ string.format("%s: %s", string.gsub(node.reference, "!", ""), node.title), node.webUrl }
		)
	end
	...
	local branch_picker = function(opts)
		...
		pickers
			.new(opts, {
				prompt_title = "Browse merge request",
